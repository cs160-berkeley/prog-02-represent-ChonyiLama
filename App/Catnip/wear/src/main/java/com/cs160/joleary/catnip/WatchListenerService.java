...
public class WatchListenerService extends WearableListenerService {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if (messageEvent.getPath().equalsIgnoreCase(ZIPCODE_PATH)) {
            String zipCodeValue = new String(messageEvent.getData(), StandardCharsets.UTF_8);
            Intent zipIntent = new Intent(this, MainActivity.class);
            zipIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            zipIntent.putExtra("ZIP_CODE", zipCodeValue);
            Log.d("T", "about to die");
